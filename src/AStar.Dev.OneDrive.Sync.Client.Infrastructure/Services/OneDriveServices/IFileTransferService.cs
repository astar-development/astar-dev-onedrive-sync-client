...
public interface IFileTransferService
{
    ...
    Task<(int CompletedFiles, long CompletedBytes)> ExecuteUploadsAsync(string accountId, HashedAccountId hashedAccountId, IReadOnlyList<DriveItemEntity> existingItems, List<FileMetadata> filesToUpload,
        int maxParallelUploads, int conflictCount, int totalFiles, long totalBytes, long uploadBytes, int completedFiles, long completedBytes, string? currentScanningFolder, Guid? sessionId,
        Action<string, HashedAccountId, SyncStatus, int, int, long, long, int, int, int, int, string?, Guid?, long?> progressReporter, CancellationTokenSource cancellationSource,
    ...
    Task<(int CompletedFiles, long CompletedBytes)> ExecuteDownloadsAsync(string accountId, HashedAccountId hashedAccountId, IReadOnlyList<DriveItemEntity> existingItems, List<FileMetadata> filesToDownload,
        int maxParallelDownloads, int conflictCount, int totalFiles, long totalBytes, long uploadBytes, long downloadBytes, int completedFiles, long completedBytes, string? currentScanningFolder, Guid? sessionId,
        Action<string, HashedAccountId, SyncStatus, int, int, long, long, int, int, int, int, string?, Guid?, long?> progressReporter, CancellationTokenSource cancellationSource,
        CancellationToken cancellationToken);
}
