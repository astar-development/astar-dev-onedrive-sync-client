...
public interface IDeletionSyncService
{
    ...
    Task ProcessRemoteToLocalDeletionsAsync(HashedAccountId hashedAccountId, IReadOnlyList<DriveItemEntity> existingFiles, HashSet<string> remotePathsSet, HashSet<string> localPathsSet, CancellationToken cancellationToken);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    Task CleanupDatabaseRecordsAsync(HashedAccountId hashedAccountId, IEnumerable<DriveItemEntity> itemsToDelete, CancellationToken cancellationToken);
}
