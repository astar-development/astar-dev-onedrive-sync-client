...
internal sealed class StrongIdModel(string? namepsace, string modelName, Accessibility accessibility, string underlyingTypeDisplay)
{
    ...
    public static bool Equals(StrongIdModel? x, StrongIdModel? y) => ReferenceEquals(x, y) || (x is not null && y is not null && string.Equals(x.Namespace, y.Namespace, StringComparison.Ordinal) &&
