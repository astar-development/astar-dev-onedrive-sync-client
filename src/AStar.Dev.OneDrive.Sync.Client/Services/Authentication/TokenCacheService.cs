...
namespace AStar.Dev.OneDrive.Sync.Client.Services.Authentication;
...
public sealed class TokenCacheService
{
    ...
    private static string GetPlatformCacheDirectory()
    {
        ...
        return OperatingSystem.IsWindows()
            ? appData.CombinePath(AppName)
            : LinuxOrMacOsGetPlatformDirectory(appData);
    }

    private static string LinuxOrMacOsGetPlatformDirectory(string appData) => OperatingSystem.IsMacOS()
        ? appData.CombinePath("Library", "Application Support", AppName)
        : appData.CombinePath(".config", AppName);
}
