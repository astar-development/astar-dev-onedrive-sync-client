...
public class AuthenticationService(IPublicClientApplication publicClientApp, ISecureTokenStorage secureTokenStorage, ILogger<AuthenticationService> logger) : IAuthenticationService
{
    private const int AuthenticationTimeoutSeconds = 30;
    private const string GraphScope = "https://graph.microsoft.com/.default";
    private readonly IPublicClientApplication _publicClientApp = publicClientApp ?? throw new ArgumentNullException(nameof(publicClientApp));
    ...
    /// <inheritdoc/>
    public async Task<Result<AuthToken, AuthenticationError>> RefreshTokenAsync(string accountIdentifier, CancellationToken cancellationToken = default)
    {
        ...
        for(var attempt = 0; attempt < maxRetries; attempt++)
        {
            try
            {
                ...
                if(account is null)
                {
                    return new Result<AuthToken, AuthenticationError>.Error(new AuthenticationError.ConfigurationError("No cached account found for silent authentication"));
                }

                AcquireTokenSilentParameterBuilder builder = _publicClientApp.AcquireTokenSilent([GraphScope], account);

                AuthenticationResult authResult = await builder.ExecuteAsync(cancellationToken);

                _logger.LogInformation("Token refreshed successfully");

                return new Result<AuthToken, AuthenticationError>.Ok(new AuthToken(authResult.AccessToken, authResult.ExpiresOn.UtcDateTime));
            }
            ...
            catch(HttpRequestException ex)
            {
                ...
                return new Result<AuthToken, AuthenticationError>.Error(new AuthenticationError.NetworkError(ex.Message));
            }
            catch(Exception ex)
            {
                _logger.LogError(ex, "Unexpected error during token refresh");

                return new Result<AuthToken, AuthenticationError>.Error(new AuthenticationError.UnexpectedError(ex.Message));
            }
        }

        return new Result<AuthToken, AuthenticationError>.Error(new AuthenticationError.UnexpectedError("Token refresh failed after maximum retries"));
    }

    private static Task DisplayDeviceCodeAsync(DeviceCodeResult deviceCodeResult) => Task.CompletedTask;
