...
- **Coverage**: Aim for high test coverage on critical business logic, but do not sacrifice code quality or design for the sake of coverage metrics. Tests should be meaningful and maintainable. Test coverage should be 80% or higher for core services and repositories, with lower thresholds for UI components where appropriate. Coverage should be branch-based, not line-based, to ensure all code paths are tested wherever practicable.
