...
public class AuthenticationServiceShould
{
    ...
    {
        var mockPublicClient = Substitute.For<IPublicClientApplication>();
        var mockSecureStorage = Substitute.For<ISecureTokenStorage>();
        var mockLogger = Substitute.For<ILogger<AuthenticationService>>();
        var mockBuilder = Substitute.For<AcquireTokenWithDeviceCodeParameterBuilder>();

        var authResult = Substitute.For<AuthenticationResult>("token123", false, string.Empty, DateTimeOffset.UtcNow.AddHours(1), DateTimeOffset.UtcNow.AddHours(1), string.Empty, null, string.Empty, [], Guid.Empty);
    ...
    public async Task ReturnTimedOutErrorWhenAuthenticateAsyncExceedsTimeout()
    {
        ...
        mockBuilder.ExecuteAsync(Arg.Any<CancellationToken>())
            .Returns(Task.Delay(TimeSpan.FromSeconds(35)).ContinueWith(_ => throw new OperationCanceledException()));
    ...
    public async Task ReturnOkResultWithRefreshedTokenWhenRefreshTokenAsyncSucceeds()
    {
        ...
        var mockSecureStorage = Substitute.For<ISecureTokenStorage>();
        var mockLogger = Substitute.For<ILogger<AuthenticationService>>();
        var mockBuilder = Substitute.For<AcquireTokenSilentParameterBuilder>();
        var mockAccount = Substitute.For<IAccount>();

        var authResult = Substitute.For<AuthenticationResult>("new_token", false, string.Empty, DateTimeOffset.UtcNow.AddHours(1), DateTimeOffset.UtcNow.AddHours(1), string.Empty, null, string.Empty, [], Guid.Empty);
        ...
        var result = await service.RefreshTokenAsync("account@example.com");
