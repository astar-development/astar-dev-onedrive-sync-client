...
public class FolderTreeServiceShould
{
    ...
    public async Task ReturnEmptyListWhenAccountIsNotAuthenticated()
    {
        ...
        _ = mockAuth.IsAuthenticatedAsync("account1", Arg.Any<CancellationToken>()).Returns(Task.FromResult<Result<bool, ErrorResponse>>(new Result<bool, ErrorResponse>.Ok(false)));
    ...
    public async Task ReturnRootFoldersWhenAuthenticated()
    {
        IGraphApiClient mockGraph = Substitute.For<IGraphApiClient>();
        IAuthService mockAuth = Substitute.For<IAuthService>();
        _ = mockAuth.IsAuthenticatedAsync("account1", Arg.Any<CancellationToken>()).Returns(Task.FromResult<Result<bool, ErrorResponse>>(new Result<bool, ErrorResponse>.Ok(true)));
    ...
    public async Task GetFolderHierarchyCallsGetRootFoldersAsync()
    {
        IGraphApiClient mockGraph = Substitute.For<IGraphApiClient>();
        IAuthService mockAuth = Substitute.For<IAuthService>();
        _ = mockAuth.IsAuthenticatedAsync(Arg.Any<string>(), Arg.Any<CancellationToken>()).Returns(Task.FromResult<Result<bool, ErrorResponse>>(new Result<bool, ErrorResponse>.Ok(true)));
