...
namespace AStar.Dev.OneDrive.Sync.Client.Tests.Integration.Features.Authentication.Services;

/// <summary>
/// Integration tests for AuthenticationService.
/// 
/// NOTE: These tests are in the Integration project (not Unit tests) because:
/// - MSAL's AcquireTokenWithDeviceCodeParameterBuilder and AcquireTokenSilentParameterBuilder 
///   are sealed classes and cannot be proxied by NSubstitute/Castle DynamicProxy.
/// - True unit testing of auth flows would require non-public MSAL APIs or test doubles
///   that don't exist in the public API surface.
/// - Integration tests with real MSAL clients or service mocks are the practical approach.
///
/// Future work: Replace with real MSAL testing using:
/// - Device Code test harness (mock browser authorization)
/// - TestContainers or in-memory services for OAuth server simulation
/// - Real MSAL client with test tenant app registration
/// </summary>
public class AuthenticationServiceShould
{
    ...
    public async Task ReturnOkResultWithAuthTokenWhenAuthenticateAsyncSucceeds()
    {
        ...
        mockPublicClient.AcquireTokenWithDeviceCode(Arg.Any<IEnumerable<string>>(), Arg.Any<Func<DeviceCodeResult, Task>>()).Returns(mockBuilder);
    ...
    public async Task ReturnTimedOutErrorWhenAuthenticateAsyncExceedsTimeout()
    {
        ...
        mockBuilder.ExecuteAsync(Arg.Any<CancellationToken>()).Returns(_ => Task.Delay(TimeSpan.FromSeconds(35)).ContinueWith(_ => throw new OperationCanceledException()));
        mockPublicClient.AcquireTokenWithDeviceCode(Arg.Any<IEnumerable<string>>(), Arg.Any<Func<DeviceCodeResult, Task>>()).Returns(mockBuilder);
    ...
    [Fact]
    public async Task ReturnCancelledErrorWhenAuthenticateAsyncIsCancelled()
    {
        IPublicClientApplication mockPublicClient = Substitute.For<IPublicClientApplication>();
        ISecureTokenStorage mockSecureStorage = Substitute.For<ISecureTokenStorage>();
        ILogger<AuthenticationService> mockLogger = Substitute.For<ILogger<AuthenticationService>>();
        ...
        mockBuilder.ExecuteAsync(Arg.Any<CancellationToken>()).Returns(_ => Task.FromException<AuthenticationResult>(new OperationCanceledException()));
        mockPublicClient.AcquireTokenWithDeviceCode(Arg.Any<IEnumerable<string>>(), Arg.Any<Func<DeviceCodeResult, Task>>()).Returns(mockBuilder);
    ...
    public async Task ReturnOkResultWithRefreshedTokenWhenRefreshTokenAsyncSucceeds()
    {
        ...
        mockPublicClient.AcquireTokenSilent(Arg.Any<IEnumerable<string>>(), Arg.Any<IAccount>()).Returns(mockBuilder);
    ...
    public async Task ReturnServiceErrorWhenAuthenticateAsyncFailsWithMsalException()
    {
        ...
        mockBuilder.ExecuteAsync(Arg.Any<CancellationToken>()).Returns(_ => Task.FromException<AuthenticationResult>(exception));
        mockPublicClient.AcquireTokenWithDeviceCode(Arg.Any<IEnumerable<string>>(), Arg.Any<Func<DeviceCodeResult, Task>>()).Returns(mockBuilder);
