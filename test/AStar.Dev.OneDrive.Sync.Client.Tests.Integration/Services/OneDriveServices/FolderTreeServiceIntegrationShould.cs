...
namespace AStar.Dev.OneDrive.Sync.Client.Tests.Integration.Services.OneDriveServices;
...
public class FolderTreeServiceIntegrationShould
{
    ...
    public async Task GetRootFoldersFromRealOneDriveAccount()
    {
        ...
        Result<AuthenticationResult, ErrorResponse> result = await authService.LoginAsync(TestContext.Current.CancellationToken);

        AuthenticationResult loginResult = result.Match(
            authResult => authResult,
            error => throw new InvalidOperationException($"Failed to authenticate with OneDrive: {error.Message}"));
        ...
        IReadOnlyList<OneDriveFolderNode> folders = await service.GetRootFoldersAsync(loginResult.AccountId, new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)), TestContext.Current.CancellationToken);
    ...
    public async Task GetChildFoldersFromRealOneDriveFolder()
    {
        ...
        IReadOnlyList<OneDriveFolderNode> rootFolders = await service.GetRootFoldersAsync(loginResult.AccountId, new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)), TestContext.Current.CancellationToken);
    ...
    public async Task GetFolderHierarchyWithLimitedDepth()
    {
        ...
        IReadOnlyList<OneDriveFolderNode> hierarchy = await service.GetFolderHierarchyAsync(loginResult.AccountId,new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)), 2, TestContext.Current.CancellationToken);
    ...
    public async Task HandleEmptyFoldersGracefully()
    {
        ...
        {
            IReadOnlyList<OneDriveFolderNode> children = await service.GetChildFoldersAsync(loginResult.AccountId,new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)), folder.DriveItemId, Arg.Any<bool?>(), TestContext.Current.CancellationToken);
    ...
    public async Task GraphApiClientCanAccessDrive()
    {
        ...
        Drive? drive = await graphApiClient.GetMyDriveAsync(loginResult.AccountId, new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)), TestContext.Current.CancellationToken);
        DriveItem? root = await graphApiClient.GetDriveRootAsync(loginResult.AccountId, new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)),TestContext.Current.CancellationToken);
        IEnumerable<DriveItem> rootChildren = await graphApiClient.GetRootChildrenAsync(loginResult.AccountId,new HashedAccountId(AccountIdHasher.Hash(loginResult.AccountId)),TestContext.Current.CancellationToken);
